...
interface HomeUiState : UiState<FragmentHomeBinding> {
    ...
    class DataError(private val message: String) : HomeUiState, UiState.SingleEvent<FragmentHomeBinding>() {
        override val block: FragmentHomeBinding.() -> Unit = {
            Toast.makeText(root.context, message, Toast.LENGTH_SHORT).show()
        }
    }

    class CantGoToCall : HomeUiState, UiState.SingleEvent<FragmentHomeBinding>() {
        override val block: FragmentHomeBinding.() -> Unit = {
