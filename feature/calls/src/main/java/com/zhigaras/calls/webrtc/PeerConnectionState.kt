...
interface PeerConnectionState {
    ...
    class Initial: PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) = Unit
    }

    class SignallingChanged(private val newState: PeerConnection.SignalingState) : PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) {
            Log.d("QQQWW signal change", newState.toString())
        }
    }

    class ConnectionChanged(private val newState: PeerConnection.PeerConnectionState) : PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) {
    ...
    class IceConnectionChanged(private val newState: PeerConnection.IceConnectionState) : PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) {
            Log.d("QQQWW ice conn change", newState.toString())
        }
    }

    class IceGatheringChanged(private val newState: PeerConnection.IceGatheringState) : PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) {
    ...
    class IceCandidatesRemoved(private val iceCandidates: Array<out IceCandidate>?) : PeerConnectionState {
        override fun handle(connectionStateHandler: ConnectionStateHandler) {
